// Language switcher — reads current locale from window.__locale (set by _i18n.js).
// On flag click: stores new locale in localStorage and reloads the page.
(function () {
    const locales = ['en', 'tr'];
    const lang = window.__locale || localStorage.getItem('locale') || 'en';
    ...
    for (const locale of locales) {
        ...
        option.setAttribute('id', `${locale}-option`);
    ...
    for (const option of dropdown.children) {
        ...
        flag.addEventListener('click', () => {
            localStorage.setItem('locale', flag.getAttribute('data-locale'));
